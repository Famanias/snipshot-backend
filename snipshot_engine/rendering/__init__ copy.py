...
def _fallback_scale_cap(region: TextBlock, severity: float) -> float:
    """Pick dynamic expansion cap for non-bubble fallback regions."""
    bubble_conf = float(getattr(region, "_bubble_confidence", 0.0) or 0.0)
    ratio = _translation_length_ratio(region)

    if bubble_conf >= 0.75:
        return 1.25 if severity > 1.25 else 1.15
    if bubble_conf >= 0.45:
        if ratio > 1.8 or severity > 1.6:
            return 2.0
        if ratio > 1.35 or severity > 1.3:
            return 1.7
        return 1.45

    # Very low confidence fallback: allow larger growth for hard pages.
    if ratio > 2.2 or severity > 1.9:
        return 2.5
    if ratio > 1.6 or severity > 1.5:
        return 2.1
    return 1.8


def _adaptive_area_growth_cap(region: TextBlock, severity: float) -> float:
    """Total area growth cap (base-bias + fallback scaling), confidence-aware."""
    bubble_conf = float(getattr(region, "_bubble_confidence", 0.0) or 0.0)
    ratio = _translation_length_ratio(region)
    ...
    if bubble_conf >= 0.75:
        return 1.8
    if bubble_conf >= 0.45:
        if severity > 1.5 or ratio > 1.8:
            return 2.6
        return 2.2

    # Low confidence fallback regions can expand much more safely.
    if severity > 1.8 or ratio > 2.0:
        return 3.5
    return 3.0


def _early_fallback_bias(region: TextBlock) -> float:
    """Early expansion factor before fit/overflow estimation when bubble detection is weak."""
    bubble_conf = float(getattr(region, "_bubble_confidence", 0.0) or 0.0)
    ratio = _translation_length_ratio(region)

    if bubble_conf < 0.35:
        return min(1.6, 1.30 + 0.20 * max(0.0, min(ratio - 1.0, 1.0)))
    if bubble_conf < 0.55:
        return min(1.4, 1.15 + 0.15 * max(0.0, min(ratio - 1.0, 1.0)))
    return 1.0
...
def _resize_regions_to_font_size(
    ...
    for region in text_regions:
        ...
        early_bias = _early_fallback_bias(region)
        if early_bias > 1.001:
            try:
                poly = Polygon(base_quad[0])
                poly = affinity.scale(poly, xfact=early_bias, yfact=early_bias, origin='center')
                pts = np.array(poly.exterior.coords[:4])
                base_quad = pts.reshape(-1, 4, 2).astype(np.int64)
                _, _, base_w, base_h = cv2.boundingRect(base_quad[0].astype(np.int32))
                base_w = max(1, int(base_w))
                base_h = max(1, int(base_h))
            except Exception:
        ...
        if region.horizontal:
            ...
            if needed_rows > used_rows:
                ...
                try:
                    ...
                    pts = np.array(poly.exterior.coords[:4])
                    dst_points = pts.reshape(-1, 4, 2).astype(np.int64)
        ...
        if not single_axis_expanded:
            ...
            # Ensure a practical minimum fallback expansion when bubble detection fails.
            min_expand = 1.0
            conf = float(getattr(region, "_bubble_confidence", 0.0) or 0.0)
            if conf < 0.35:
                min_expand = 1.40
            elif conf < 0.55:
                min_expand = 1.20

            final_scale_x = max(min_expand, min(max(base_scale, over_x), cap))
            final_scale_y = max(min_expand, min(max(base_scale, over_y), cap))

            # Keep total area growth bounded adaptively.
            total_cap = _adaptive_area_growth_cap(region, severity)
            total_growth = (early_bias * early_bias) * final_scale_x * final_scale_y
            if total_growth > total_cap:
                corr = np.sqrt(total_cap / total_growth)
                final_scale_x = max(1.0, final_scale_x * corr)
                final_scale_y = max(1.0, final_scale_y * corr)
...
def _estimate_overflow_scales(
    ...
    """Estimate anisotropic expansion scales using current available region size."""
    ...
    if region.horizontal:
        ...
        scale_x = 1.0 + 0.45 * (width_overflow - 1.0) + 0.35 * (row_overflow - 1.0)
        scale_y = 1.0 + 0.80 * (row_overflow - 1.0)
    else:
        ...
        scale_x = 1.0 + 0.80 * (col_overflow - 1.0)
        scale_y = 1.0 + 0.45 * (height_overflow - 1.0) + 0.35 * (col_overflow - 1.0)
...
def _find_optimal_font_size(
    ...
    if min_dim < 80:
        max_fs = min(int(min_dim * 0.60), 44)
    elif min_dim < 200:
        max_fs = int(44 + (min_dim - 80) * 0.35)
    else:
        max_fs = min(int(min_dim * 0.48), 180)
    
    # Relax growth caps, especially for low-confidence fallback regions.
    growth_mult = 4.0 if bubble_conf < 0.45 else 3.0
    ...
    for _ in range(14):
        ...
        if render_h:
            ...
            h_slack = 1.06 if bubble_conf < 0.45 else 1.02
            w_slack = 1.08 if bubble_conf < 0.45 else 1.03
            ...
            w_slack = 1.06 if bubble_conf < 0.45 else 1.02
            h_slack = 1.08 if bubble_conf < 0.45 else 1.03
