...
if __name__ == '__main__':
    ...
    Supports both `python server/main.py` (script mode) and
    `python -m server.main` (module mode). We attempt relative import first
    (works in module mode when `server` is a package), then fall back to
    absolute forms for script execution.
    """
    import uvicorn
    try:
        # Module mode (requires server/__init__.py and run via -m)
        from .args import parse_arguments  # type: ignore
    except ImportError:
        try:
            # Fallback if executed as script and PYTHONPATH includes project root
            from server.args import parse_arguments  # type: ignore
        except ImportError:
            # Last resort: local directory import
            from args import parse_arguments  # type: ignore
